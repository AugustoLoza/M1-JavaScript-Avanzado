...
var objContains = function (obj, prop, value) {
  ...
  for (var props in obj) {
    if (obj[props] === value && props === prop) {
      return true;
    }
    else if (typeof obj[props] === 'object') {
      var res = objContains(obj[props], prop, value);
      if (res) { return true }
    }
  }
  return false;
...
LinkedList.prototype.addInPos = function (pos, value) {
  ...
  let currentent = this.head;
  while (currentent) {
    arrayn.push(currentent.value)
    currentent = currentent.next
  }
  arrayn.splice(pos, 0, value)
...
LinkedList.prototype.removeFromPos = function (pos) {
  ...
  if (pos > arrayn.length - 1) { return false }
  var remove = arrayn.splice(pos, 1);
...
var controlAcces = function (queue, event) {
  ...
  while (queue.size()) {
    ...
    if (persona.age >= 18 && persona.ticket.event === event && !ticketingresados.includes(persona.ticket.number)) {
...
var binarySearch = function (array, elemento) {
  /* Tu codigo aqui */
  function recursiveFunction(arr, elemento, start, end) {
    if (start > end) return -1;
    let mid = Math.floor((start + end) / 2);
    if (arr[mid] === elemento) return mid;
    if (arr[mid] > elemento) {
      return recursiveFunction(arr, elemento, start, mid - 1);
    } else {
      return recursiveFunction(arr, elemento, mid + 1, end);
    }
  }
  return recursiveFunction(array, elemento, array[0], array[array.length - 1]);
